...
mod file_storage_operations {
    ...
    fn put_operation(size_kb: usize) -> color_eyre::Result<()> {
    ...
    #[divan::bench(args = [1, 10, 100, 1000])]
    fn get_operation(size_kb: usize) -> color_eyre::Result<AudioBuffer> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let storage = FileStorage::new(
    ...
    #[divan::bench]
    fn delete_operation() -> color_eyre::Result<()> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let storage = FileStorage::new(
...
mod filesystem_cache_operations {
    ...
    #[divan::bench(args = [1, 10, 100, 1000])]
    fn cache_set_operation(size_kb: usize) -> color_eyre::Result<()> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
    ...
    #[divan::bench(args = [1, 10, 100, 1000])]
    fn cache_get_operation(size_kb: usize) -> color_eyre::Result<Option<Vec<u8>>> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let cache = FileSystemCache::new(temp_dir.path()).unwrap();
    ...
    #[divan::bench]
    fn cache_delete_operation() -> color_eyre::Result<()> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let cache = FileSystemCache::new(temp_dir.path()).unwrap();
    ...
    #[divan::bench]
    fn cache_set_with_ttl() -> color_eyre::Result<()> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let cache = FileSystemCache::new(temp_dir.path()).unwrap();
    ...
    #[divan::bench]
    fn cache_get_expired() -> color_eyre::Result<Option<Vec<u8>>> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let cache = FileSystemCache::new(temp_dir.path()).unwrap();
...
mod storage_vs_cache_comparison {
    ...
    fn storage_write_performance(size_kb: usize) -> color_eyre::Result<()> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let storage = FileStorage::new(
    ...
    #[divan::bench(args = [10, 100, 1000])]
    fn cache_write_performance(size_kb: usize) -> color_eyre::Result<()> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let cache = FileSystemCache::new(temp_dir.path()).unwrap();
    ...
    #[divan::bench(args = [10, 100, 1000])]
    fn storage_read_performance(size_kb: usize) -> color_eyre::Result<AudioBuffer> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let storage = FileStorage::new(
    ...
    #[divan::bench(args = [10, 100, 1000])]
    fn cache_read_performance(size_kb: usize) -> color_eyre::Result<Option<Vec<u8>>> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let cache = FileSystemCache::new(temp_dir.path()).unwrap();
...
mod error_handling {
    ...
    #[divan::bench]
    fn storage_get_nonexistent() -> color_eyre::Result<AudioBuffer> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let storage = FileStorage::new(
    ...
    #[divan::bench]
    fn storage_delete_nonexistent() -> color_eyre::Result<()> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let storage = FileStorage::new(
    ...
    #[divan::bench]
    fn cache_get_nonexistent() -> color_eyre::Result<Option<Vec<u8>>> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let cache = FileSystemCache::new(temp_dir.path()).unwrap();
    ...
    #[divan::bench]
    fn cache_delete_nonexistent() -> color_eyre::Result<()> {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let temp_dir = tempdir().unwrap();
            let cache = FileSystemCache::new(temp_dir.path()).unwrap();
