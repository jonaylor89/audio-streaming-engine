...
fn create_realistic_params(scenario: &str) -> Params {
    match scenario {
        "podcast" => Params {
            ...
            compressor: Some("threshold=0.125:ratio=3:attack=20:release=200:makeup=1".to_string()),
            ..Default::default()
        },
        ...
        "radio_broadcast" => Params {
            ...
            highpass: Some(80.0),
            compressor: Some("threshold=0.1:ratio=4:attack=10:release=150:makeup=1".to_string()),
            ..Default::default()
        },
        "audiobook" => Params {
...
mod end_to_end_pipeline {
    ...
    #[divan::bench(
        ...
        ("gaming_audio", AudioFormat::Wav, 150),
...
mod concurrent_streaming {
    ...
    #[divan::bench(ignore = cfg!(codspeed), args = [1, 2, 4, 8, 16])]
    fn concurrent_requests(bencher: Bencher<'_, '_>, concurrency: usize) {
        ...
        let processor = Arc::new(Processor::new(ProcessorSettings {
            disabled_filters: Vec::new(),
            max_filter_ops: 100,
        ...
        let test_scenarios = [
            ...
            ("gaming_audio", AudioFormat::Wav, 40),
...
mod cache_hot_cold_paths {
    ...
    fn cache_miss_process_and_store(bencher: Bencher<'_, '_>) {
        let temp_dir = tempdir().unwrap();
        let cache_dir = temp_dir.path().join("cache");
        let storage_dir = temp_dir.path().join("storage");
        ...
        bencher.bench(|| {
            rt.block_on(async {
                let cache_key = "fixture-miss";
                cache.delete(cache_key).await.ok();

                let processed = processor
                    .process(black_box(&*SAMPLE_MP3_FIXTURE), black_box(&params))
                    .await?;
                let processed_bytes = processed.clone().into_bytes();
                storage.put("fixture-miss.wav", &processed).await?;
                cache
                    .set(cache_key, &processed_bytes, Some(Duration::from_secs(3600)))
                    .await?;

                Ok::<_, color_eyre::Report>(black_box(processed.len()))
            })
        })
    }
}
...
mod memory_pressure_scenarios {
    ...
    #[divan::bench(ignore = cfg!(codspeed), args = [1, 5, 10, 20])]
